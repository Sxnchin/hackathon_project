...
function Demo() {
  ...
  const handleAddMember = () => {
    ...
    if (memberName) {
      const newMember = {
        id: (splitState?.participants.length || 0) + 1,
        name: memberName,
        share: 100,
        status: 'pending',
      };
      const updated = {
        ...splitState,
        participants: [...(splitState?.participants || []), newMember],
      };
  ...
  if (!isSplitting) {
    actionContent = (
      <button onClick={handleStartSplit} className="start-split-btn-modern">
    ...
    <div className="demo-page-container">
      ...
      >
        <div className="demo-header">
          ...
          <p className="text-gray-500 mt-2">
            Mock Checkout: Real-time splitting of a $300 TV purchase.
          </p>
        </div>
        ...
        <div className="demo-content-grid">
          <div className="checkout-panel">
            ...
            <div className="participants-section">
              ...
              <div className="space-y-3">
                {splitState?.participants?.map((user) => (
                  <div key={user.id} className="participant-row-modern">
                    ...
                    {user.status === 'paying' && (
                      <button
                        disabled
                        className="pay-btn-modern processing flex items-center justify-center"
                      >
                        <SpinnerIcon />
                      </button>
                    )}
                ...
                <button className="start-split-btn-modern mt-4" onClick={handleAddMember}>
                  + Add Member to Group
                </button>
              </div>
            </div>
          ...
          <div className="action-panel-modern">{actionContent}</div>
        </div>
      </motion.div>
