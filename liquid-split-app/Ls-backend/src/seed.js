...
const mockSocketServer = {
  ...
  emit(event, data) {
    switch (event) {
      case 'start-split':
        this.handleStartSplit(data?.participants);
        break;
      case 'user-paid':
  ...
  handleStartSplit(customParticipants) {
    if (customParticipants && customParticipants.length) {
      this.state.participants = customParticipants;
    } else {
      this.state.participants = [
        { id: 1, name: 'You', share: 100, status: 'pending' },
        { id: 2, name: 'Alex', share: 100, status: 'paid' },
        { id: 3, name: 'Jordan', share: 100, status: 'paid' },
      ];
    }
...
function Demo() {
  ...
  // Initial group: "You", Alex, and Jordan
  ...
  /* --- Add Members (Even Split) --- */
  const recalcShares = (arr) => {
    const n = arr.length;
    if (n === 0) return [];
    const evenShare = +(300 / n).toFixed(2);
    return arr.map((p) => ({ ...p, share: evenShare }));
  };
  ...
  const handleAddMember = () => {
    ...
    if (user) {
      const updated = recalcShares([...participants, { ...user, status: "paid" }]);
      setParticipants(updated);
    }
  };

  const handleRemoveMember = (id) => {
    if (participants.length <= 3) return alert("You need at least 3 participants to start a split!");
    setParticipants(recalcShares(participants.filter(p => p.id !== id)));
  };
  ...
  const handleStartSplit = () => {
    if (participants.length < 3) {
      alert("At least 3 participants are required to split.");
      return;
    }
    const total = participants.reduce((sum, p) => sum + p.share, 0);
    if (total < 300) {
      alert("Minimum total amount must be $300.");
      return;
    }
    const readyList = recalcShares(participants);
    mockSocketServer.emit('start-split', { participants: readyList });
    setIsSplitting(true);
  };
  ...
  const handleOnboard = async (userId = 1) => {
    alert("Mock: Stripe Connect Onboard triggered for user " + userId);
  };

  const liveParticipants = isSplitting ? splitState?.participants || [] : participants;
  const paidCount = liveParticipants.filter(p => p.status === 'paid').length;
  const totalCount = liveParticipants.length;
          ...
          <div className="checkout-panel">
            ...
            <div className="participants-section">
              <h4 style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                ...
                {!isSplitting && <button onClick={handleAddMember} className="add-people action-btn-modern">Add Members</button>}
              </h4>
              <div className="space-y-3">
                {liveParticipants.map((user) => (
                  <div key={user.id} className="participant-row-modern">
                    ...
                    {!isSplitting && user.id !== 1 && participants.length > 3 && (
                      <button onClick={() => handleRemoveMember(user.id)} className="remove-member-btn">&times;</button>
                    )}
            ...
            {!isSplitting ? (
              ...
              <div className="text-center w-full">
                ...
                <div className="mt-4 action-btn-group-modern">
                  <button onClick={() => handleOnboard(1)} className="action-btn-modern">Connect Stripe</button>
                </div>
              </div>
            ) : receipts.length === 0 ? (
              <div className="text-center w-full">
                ...
                <div className="space-y-3">
                  {receipts.map((r, i) => (
                    <div key={i} className="receipt-card-modern" style={{ animationDelay: `${i * 150}ms` }}>
                      <p className="font-bold text-gray-800">{r.owner}'s Digital Receipt</p>
                      <div className="flex justify-between text-sm text-gray-600 mt-1">
                        <span>Value: ${r.value.toFixed(2)}</span>
                        <span>Ownership: {r.ownership}</span>
                      </div>
                      <p className="text-xs text-indigo-500 mt-2 font-mono">NFT ID: {r.nftId}</p>
                    </div>
                  ))}
                </div>
              </div>
